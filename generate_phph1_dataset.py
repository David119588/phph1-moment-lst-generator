...
def parse_k_list(s):
    """
    Convert string like '1,3,5,7,9' to list [1,3,5,7,9].
    """
...
if SUMMARY_EXAMPLES_CSV.exists() and RESUME_IF_FILES_EXIST:
    examples_df_existing = pd.read_csv(SUMMARY_EXAMPLES_CSV)
    done_example_ids = set(
        examples_df_existing.loc[
            examples_df_existing["status"].astype(str).str.lower() == "ok",
            "example_id"
        ].astype(int).tolist()
    )

    print("Loaded existing examples summary.")
    print("Already completed examples:", len(done_example_ids))

else:
    examples_df_existing = pd.DataFrame()
    done_example_ids = set()

if SUMMARY_FITS_CSV.exists() and RESUME_IF_FILES_EXIST:
    fits_df_existing = pd.read_csv(SUMMARY_FITS_CSV)
else:
    fits_df_existing = pd.DataFrame()

if FAILURES_CSV.exists() and RESUME_IF_FILES_EXIST:
    failures_df_existing = pd.read_csv(FAILURES_CSV)
else:
    failures_df_existing = pd.DataFrame()
...
for _, plan_row in tqdm(plan_df.iterrows(), total=len(plan_df), desc="Generating PH/PH/1 examples"):

    example_id = int(plan_row["example_id"])

    if example_id in done_example_ids:
        continue

    try:
        t0 = time.time()

        arrival_pool_id = int(plan_row["arrival_pool_id"])
        service_pool_id = int(plan_row["service_pool_id"])
        rho_target = float(plan_row["rho_target"])

        arrival_item = arrival_pool_by_id[arrival_pool_id]
        service_item = service_pool_by_id[service_pool_id]

        # ----------------------------------------------------
        # Build queue and true sojourn-time ME
        # ----------------------------------------------------

        qobj = phph1_sojourn_me_from_arrival_service_items(
            arrival_item=arrival_item,
            service_item=service_item,
            rho_target=rho_target,
        )

        beta_W = as_row(qobj["beta"])
        T_W = as_mat(qobj["T"])

        alpha_A = as_row(qobj["arrival_alpha"])
        T_A = as_mat(qobj["arrival_T"])

        alpha_S = as_row(qobj["service_alpha"])
        T_S = as_mat(qobj["service_T"])

        rho_actual = float(qobj["rho"])

        arrival_family = qobj["arrival_family"]
        service_family = qobj["service_family"]

        arrival_scv = float(qobj["arrival_scv"])
        service_scv = float(qobj["service_scv"])

        sojourn_me_dim = int(T_W.shape[0])

        # ----------------------------------------------------
        # Moments
        # ----------------------------------------------------

        arrival_moms_20 = ph_moments(alpha_A, T_A, NUM_MOMENTS_SAVE)
        service_moms_20 = ph_moments(alpha_S, T_S, NUM_MOMENTS_SAVE)
        sojourn_moms_20 = ph_moments(beta_W, T_W, NUM_MOMENTS_SAVE)

        arrival_moms_norm_20 = normalized_moments(arrival_moms_20)
        service_moms_norm_20 = normalized_moments(service_moms_20)
        sojourn_moms_norm_20 = normalized_moments(sojourn_moms_20)

        mean_sojourn = float(sojourn_moms_20[0])

        # ----------------------------------------------------
        # True LST
        # ----------------------------------------------------
        # raw LST: L_W(s)
        # normalized LST: L_{W/EW}(s) = L_W(s / E[W])

        true_lst_raw = lst_me_points(beta_W, T_W, S_GRID)
        true_lst_norm = lst_me_points(beta_W, T_W, S_GRID / mean_sojourn)

        # ----------------------------------------------------
        # Fit LSTs using first K normalized sojourn moments
        # ----------------------------------------------------
        ...
        for K in K_LIST:
            ...
            fit_obj = {
                ...
                "l2_norm": np.nan,
                "sup_err_norm": np.nan,
                "moment_fit_max_pct_error": np.nan,
                "fit_status": "not_run",
            }

            try:
                input_moms = sojourn_moms_norm_20[:K].tolist()

                alpha_fit, T_fit = MEFromMoments(input_moms)

                alpha_fit = as_row(alpha_fit)
                T_fit = as_mat(T_fit)

                fit_lst_norm = lst_me_points(alpha_fit, T_fit, S_GRID)
                ...
                max_mom_err = max_percent_moment_error(
                    alpha_fit,
                    T_fit,
                    target_moms=input_moms,
                )

                validity = check_lst_validity(fit_lst_norm)

                fit_obj.update({
                    "alpha_fit": alpha_fit,
                    "T_fit": T_fit,
                    "fit_lst_norm": fit_lst_norm,
                    "l2_norm": l2_norm,
                    "sup_err_norm": sup_norm,
                    "moment_fit_max_pct_error": max_mom_err,
                    "fit_status": "ok",
                })

                fit_obj.update(validity)

            except Exception as e_fit:
                fit_obj["fit_status"] = f"fit_fail: {type(e_fit).__name__}: {e_fit}"

            fit_results[K] = fit_obj

            fits_rows_new.append({
                "example_id": example_id,
                "K": K,
                "rho": rho_actual,
                ...
                "arrival_pool_id": arrival_pool_id,
                "service_pool_id": service_pool_id,
                "sojourn_me_dim": sojourn_me_dim,
                "mean_sojourn": mean_sojourn,

                "l2_norm": fit_obj.get("l2_norm", np.nan),
                "sup_err_norm": fit_obj.get("sup_err_norm", np.nan),
                "moment_fit_max_pct_error": fit_obj.get("moment_fit_max_pct_error", np.nan),

                "fit_status": fit_obj.get("fit_status", "unknown"),
                "fit_valid": fit_obj.get("fit_valid", False),
                "is_finite": fit_obj.get("is_finite", False),
                "is_in_0_1": fit_obj.get("is_in_0_1", False),
                "starts_at_1": fit_obj.get("starts_at_1", False),
                "is_monotone": fit_obj.get("is_monotone", False),

                "lst_min": fit_obj.get("lst_min", np.nan),
                "lst_max": fit_obj.get("lst_max", np.nan),
                "lst_at_0": fit_obj.get("lst_at_0", np.nan),
                "max_upward_jump": fit_obj.get("max_upward_jump", np.nan),
                "num_bound_violations": fit_obj.get("num_bound_violations", np.nan),
                "num_monotone_violations": fit_obj.get("num_monotone_violations", np.nan),
            })

        # ----------------------------------------------------
        # Save full PKL for the example
        # ----------------------------------------------------
        ...
        example_obj = {
            ...
            "arrival_size": ARRIVAL_PH_SIZE,
            "service_size": SERVICE_PH_SIZE,
            "state_product": ARRIVAL_PH_SIZE * SERVICE_PH_SIZE,
            ...
            # PH representations used in queue
            "arrival_alpha": alpha_A,
            "arrival_T": T_A,
            "service_alpha": alpha_S,
            "service_T": T_S,
            ...
            # raw moments
            "arrival_moments_1_to_20": arrival_moms_20,
            "service_moments_1_to_20": service_moms_20,
            "sojourn_moments_1_to_20": sojourn_moms_20,

            # normalized moments
            "arrival_moments_norm_1_to_20": arrival_moms_norm_20,
            "service_moments_norm_1_to_20": service_moms_norm_20,
            "sojourn_moments_norm_1_to_20": sojourn_moms_norm_20,

            # LSTs
            "s_grid": S_GRID,
            "true_lst_raw": true_lst_raw,
            "true_lst_norm": true_lst_norm,

            # fit results for K
            "fit_results": fit_results,
        }

        example_pkl_path = EXAMPLES_DIR / f"example_{example_id:06d}.pkl"

        with open(example_pkl_path, "wb") as f:
            pickle.dump(example_obj, f)

        # ----------------------------------------------------
        # Summary row for this example
        # ----------------------------------------------------
        ...
        ex_row = {
            ...
            "pkl_path": str(example_pkl_path),
            "elapsed_sec": elapsed,
            "status": "ok",
        }

        # raw moments as columns
        for k in range(1, NUM_MOMENTS_SAVE + 1):
            ex_row[f"arrival_m{k}"] = float(arrival_moms_20[k - 1])
            ex_row[f"service_m{k}"] = float(service_moms_20[k - 1])
            ex_row[f"sojourn_m{k}"] = float(sojourn_moms_20[k - 1])

            ex_row[f"arrival_norm_m{k}"] = float(arrival_moms_norm_20[k - 1])
            ex_row[f"service_norm_m{k}"] = float(service_moms_norm_20[k - 1])
            ex_row[f"sojourn_norm_m{k}"] = float(sojourn_moms_norm_20[k - 1])

        examples_rows_new.append(ex_row)

        done_example_ids.add(example_id)

    except Exception as e:

        failures_rows_new.append({
            "example_id": example_id,
            "arrival_pool_id": plan_row.get("arrival_pool_id", np.nan),
            "service_pool_id": plan_row.get("service_pool_id", np.nan),
            "rho_target": plan_row.get("rho_target", np.nan),
            "status": f"fail: {type(e).__name__}: {e}",
        })

    # --------------------------------------------------------
    # Save progressively after each example
    # --------------------------------------------------------

    examples_df_current = pd.concat(
        [examples_df_existing, pd.DataFrame(examples_rows_new)],
        ignore_index=True
    )

    fits_df_current = pd.concat(
        [fits_df_existing, pd.DataFrame(fits_rows_new)],
        ignore_index=True
    )

    failures_df_current = pd.concat(
        [failures_df_existing, pd.DataFrame(failures_rows_new)],
        ignore_index=True
    )

    if len(examples_df_current) > 0:
        examples_df_current = (
            examples_df_current
    ...
    if len(fits_df_current) > 0:
        fits_df_current = (
            fits_df_current
            .drop_duplicates(subset=["example_id", "K"], keep="last")
            .sort_values(["example_id", "K"])
            .reset_index(drop=True)
        )

    examples_df_current.to_csv(SUMMARY_EXAMPLES_CSV, index=False)
    fits_df_current.to_csv(SUMMARY_FITS_CSV, index=False)
    failures_df_current.to_csv(FAILURES_CSV, index=False)

print("Finished main loop.")

examples_df = pd.read_csv(SUMMARY_EXAMPLES_CSV) if SUMMARY_EXAMPLES_CSV.exists() else pd.DataFrame()
fits_df = pd.read_csv(SUMMARY_FITS_CSV) if SUMMARY_FITS_CSV.exists() else pd.DataFrame()
failures_df = pd.read_csv(FAILURES_CSV) if FAILURES_CSV.exists() else pd.DataFrame()

print("examples_df shape:", examples_df.shape)
print("fits_df shape:", fits_df.shape)
print("failures_df shape:", failures_df.shape)

display(examples_df.tail())
display(fits_df.tail())
display(failures_df.tail())
